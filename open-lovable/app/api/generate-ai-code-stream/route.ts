      ...
      try {
        ...
        if (isEdit) {
          ...
          if (manifest) {
            ...
            if (!fileContents) {
              console.warn('[generate-ai-code-stream] No file cache available');
              return;
            }
...
5. Make the requested change immediately`;
        ...
        if (isAnthropic) {
          ...
          // Moonshot models use Groq provider for now
          modelProvider = groq;
          actualModel = model.replace('moonshotai/', '');
        } else {
          modelProvider = groq;
          actualModel = model;
        }
...
It's better to have 3 complete files than 10 incomplete files.`
        ...
        // Add temperature for non-reasoning models (all except GPT-5)
        if (!model.startsWith('openai/gpt-5')) {
          streamOptions.temperature = appConfig.ai.defaultTemperature;
        }
        
        // Add reasoning effort for GPT-5 models only
        if (isOpenAI && model.startsWith('openai/gpt-5')) {
